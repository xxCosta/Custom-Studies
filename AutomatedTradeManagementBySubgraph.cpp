...
SCSFExport scsf_TradeManagementByStudy(SCStudyInterfaceRef sc)
{
	...
	SCDateTime& r_LastAdjustmentDateTime = sc.GetPersistentSCDateTime(1);
	...
	{
		r_LastAdjustmentDateTime.Clear();
	...
	const bool AdjustByFixedTimeInSeconds = Input_AdjustmentTiming.GetInt() == 0;

	// If the timing is every N seconds, check that we have surpassed the set frequency
	if (AdjustByFixedTimeInSeconds)
	{
		if (r_LastAdjustmentDateTime.IsUnset())
		{
			r_LastAdjustmentDateTime = sc.LatestDateTimeForLastBar;
		}

		//  Using absolute value because there could be an unexpected change in time with sc.LatestDateTimeForLastBar, for numerous reasons, causing a negative value result of this calculation.
		const SCDateTime TimeDifference = (sc.LatestDateTimeForLastBar - r_LastAdjustmentDateTime).GetAbsoluteValue();

		if (TimeDifference < SCDateTime::SECONDS_Fraction(Input_AdjustmentFrequency.GetFloat()))
		{
	...
	// Update the LastAdjustmentDateTime even if we are not relying on it
	r_LastAdjustmentDateTime = sc.LatestDateTimeForLastBar;
