...
SCSFExport scsf_VolumeWeightedAveragePrice(SCStudyInterfaceRef sc)
{
	...
	SCInputRef Input_IgnoreTimePeriodTypeAndLength = sc.Input[n_ACSIL::VWAP_IGNORE_TIME_PERIOD_TYPE_AND_LENGTH_INPUT_INDEX];
	SCInputRef Input_BaseOnUnderlyingData   = sc.Input[9];
	SCInputRef Input_UseFixedOffset         = sc.Input[10];
	SCInputRef Input_StartDateTime			= sc.Input[n_ACSIL::VWAP_START_DATE_TIME_INPUT_INDEX];
	...
	SCInputRef Input_DrawnWithAnchoredVWAPTool = sc.Input[n_ACSIL::VWAP_DRAWN_WITH_ANCHORED_VWAP_TOOL_INPUT_INDEX];

	SCInputRef Input_FullRecalculate = sc.Input[n_ACSIL::VWAP_FULL_RECALCULATE_INPUT_INDEX];
	
	const int StudyVersion = 7;
	...
	if(sc.SetDefaults)
	{
		...
		Input_DrawnWithAnchoredVWAPTool.Name = "";//Hidden
		Input_DrawnWithAnchoredVWAPTool.SetYesNo(0);
		
		Input_FullRecalculate.Name = "";//Hidden
		Input_FullRecalculate.SetYesNo(0);

		Input_Version.SetInt(StudyVersion);

		return;
	}

	if (Input_Version.GetInt() < 7)
	{
		// Nothing to do.
	}
	...
	{
		if (Input_DrawnWithAnchoredVWAPTool.GetYesNo())
		{
			sc.DeleteChartDrawingByDrawnStudyInstanceID(sc.StudyGraphInstanceID);
		}
	...
	if (sc.IsFullRecalculation)
	{
		Array_BarSumPriceTimesVolume[0] = 0;
		Array_BarSumPriceSquaredTimesVolume[0] = 0;
	...
	int StartingIndex = sc.UpdateStartIndex;

	if (Input_FullRecalculate.GetYesNo())
	{
		Input_FullRecalculate.SetYesNo(false);
		StartingIndex = 0;
	...
	for (int BarIndex = StartingIndex; BarIndex < sc.ArraySize; ++BarIndex)
	{

		if (Input_StartDateTime.GetDate() != 0
			&& sc.BaseDateTimeIn[BarIndex] < Input_StartDateTime.GetDateTime())
		{
			Subgraph_VWAPSubgraph[BarIndex] = 0;

			Subgraph_SD1Top[BarIndex] = 0;
			Subgraph_SD1Bottom[BarIndex] = 0;
			Subgraph_SD2Top[BarIndex] = 0;
			Subgraph_SD2Bottom[BarIndex] = 0;
			Subgraph_SD3Top[BarIndex] = 0;
			Subgraph_SD3Bottom[BarIndex] = 0;
			Subgraph_SD4Top[BarIndex] = 0;
			Subgraph_SD4Bottom[BarIndex] = 0;

			continue;
		}

		bool PriorBarIndexExcludedBasedOnDateTime = BarIndex > 0
			&& Input_StartDateTime.GetDate() != 0
			&& sc.BaseDateTimeIn[BarIndex - 1] < Input_StartDateTime.GetDateTime();
			...
			|| PriorBarIndexExcludedBasedOnDateTime)
		{
			...
			r_LastIndexProcessed = BarIndex;

			r_StartIndexOfCurrentPeriod = BarIndex;
			IsStartOfNewPeriod = true;
		}
		else if (BarIndex != r_LastIndexProcessed)
		{
			// add in last values
			for (; r_LastIndexProcessed < BarIndex; ++r_LastIndexProcessed)
			{
		...
		float CurrentPrice = sc.BaseDataIn[Input_InputData.GetInputDataIndex()][BarIndex];

		if (Input_BaseOnUnderlyingData.GetYesNo())
		{
			Array_BarSumPriceTimesVolume[BarIndex] = 0;
			Array_BarSumPriceSquaredTimesVolume[BarIndex] = 0;
			...
			const int NumVAPElementsAtBarIndex = sc.VolumeAtPriceForBars->GetSizeAtBarIndex(BarIndex);
			for (int VAPIndex = 0; VAPIndex < NumVAPElementsAtBarIndex; VAPIndex++)
			{
				sc.VolumeAtPriceForBars->GetVAPElementAtIndex(BarIndex, VAPIndex, &p_VolumeAtPrice);
				...
				Array_BarSumPriceTimesVolume[BarIndex] += (Price * VolumeToUse);
				Array_BarSumPriceSquaredTimesVolume[BarIndex] += (Price * Price * VolumeToUse);
		...
		else
		{
			...
			Array_BarSumPriceTimesVolume[BarIndex] = CurrentPrice * sc.Volume[BarIndex];
			Array_BarSumPriceSquaredTimesVolume[BarIndex] = CurrentPrice * CurrentPrice * sc.Volume[BarIndex];
			BarVolume = sc.Volume[BarIndex];
		}

		Array_CalculatedBarVolume[BarIndex] = static_cast <float> (BarVolume);

		double PriceTimesVolume = r_PriorSumPriceTimesVolume + Array_BarSumPriceTimesVolume[BarIndex];
		double PriceSquaredTimesVolume = r_PriorSumPriceSquaredTimesVolume + Array_BarSumPriceSquaredTimesVolume[BarIndex];
		...
		Subgraph_VWAPSubgraph[BarIndex] = static_cast<float>(VolumeWeightedAveragePrice);
		...
		else if (Input_BandCalculationMethod.GetIndex() == 0)//VWAP variance
		{
			...
			{
				Array_BarSumPriceVWAPDifferenceSquaredTimesVolume[BarIndex] =
					static_cast<float>(Array_BarSumPriceVWAPDifferenceSquaredTimesVolume[BarIndex - 1] + PriceVWAPDifferenceSquaredTimesVolume);
			}
		...
		Subgraph_SD2Bottom[BarIndex] = Subgraph_VWAPSubgraph[BarIndex] - Input_Band2OffsetMultiplier.GetFloat() * BandDistance;

		Subgraph_SD3Top[BarIndex] = Subgraph_VWAPSubgraph[BarIndex] + Input_Band3OffsetMultiplier.GetFloat() * BandDistance;
		Subgraph_SD3Bottom[BarIndex] = Subgraph_VWAPSubgraph[BarIndex] - Input_Band3OffsetMultiplier.GetFloat() * BandDistance;

		Subgraph_SD4Top[BarIndex] = Subgraph_VWAPSubgraph[BarIndex] + Input_Band4OffsetMultiplier.GetFloat() * BandDistance;
		Subgraph_SD4Bottom[BarIndex] = Subgraph_VWAPSubgraph[BarIndex] - Input_Band4OffsetMultiplier.GetFloat() * BandDistance;
	...
	for (int Index = sc.UpdateStartIndex; Index < sc.ArraySize; ++Index)
	{
		...
		for (int AvgIndex = CalculationStartIndex; AvgIndex <= Index; ++AvgIndex)
		{
			...
			else
			{
				...
				if (VolumeTypeToUse == 1)
				{
					Subgraph_CumulativeVolumeForPeriodArray[AvgIndex] = Subgraph_CumulativeVolumeForPeriodArray[AvgIndex - 1] + sc.BidVolume[AvgIndex];
				}
				else if (VolumeTypeToUse == 2)
				{
					Subgraph_CumulativeVolumeForPeriodArray[AvgIndex] = Subgraph_CumulativeVolumeForPeriodArray[AvgIndex - 1] + sc.AskVolume[AvgIndex];
				}
				else//Total volume
				{
...
/*==========================================================================*/
SCSFExport scsf_TextDisplayFromSpreadsheet(SCStudyInterfaceRef sc)
{
	SCSubgraphRef Subgraph_TextDisplay = sc.Subgraph[0];

	SCInputRef Input_HorizontalPosition = sc.Input[0];
	SCInputRef Input_VerticalPosition = sc.Input[1];
	SCInputRef Input_SpreadsheetName = sc.Input[2];
	SCInputRef Input_SheetName = sc.Input[3];
	SCInputRef Input_CellReference = sc.Input[4];

	if (sc.SetDefaults)
	{
		// Set the configuration and defaults

		sc.GraphName = "Text Display from Spreadsheet";
		...
		Input_HorizontalPosition.Name.Format("Horizontal Position From Left (1-%d)", static_cast<int>(CHART_DRAWING_MAX_HORIZONTAL_AXIS_RELATIVE_POSITION));
		Input_HorizontalPosition.SetInt(5);
		Input_HorizontalPosition.SetIntLimits(1, static_cast<int>(CHART_DRAWING_MAX_HORIZONTAL_AXIS_RELATIVE_POSITION));

		Input_VerticalPosition.Name.Format("Vertical Position From Bottom (1-%d)", static_cast<int>(CHART_DRAWING_MAX_VERTICAL_AXIS_RELATIVE_POSITION));
		Input_VerticalPosition.SetInt(95);
		Input_VerticalPosition.SetIntLimits(1, static_cast<int>(CHART_DRAWING_MAX_VERTICAL_AXIS_RELATIVE_POSITION));

		Input_SpreadsheetName.Name = "Spreadsheet Name";
		Input_SpreadsheetName.SetString("");

		Input_SheetName.Name = "Sheet Name";
		Input_SheetName.SetString("Sheet1");

		Input_CellReference.Name = "Cell Reference";
		Input_CellReference.SetString("A1");
	...
	void* SheetHandle
		= sc.GetSpreadsheetSheetHandleByName
			( Input_SpreadsheetName.GetString()
			, Input_SheetName.GetString()
			, false
			);
	...
	sc.GetSpreadsheetColumnAndRowIndexesFromCellReference
		( Input_CellReference.GetString()
		, &ColumnIndex
		, &RowIndex
		);

	SCString CellText;
	sc.GetSheetCellAsString(SheetHandle, ColumnIndex, RowIndex, CellText);
...
SCSFExport scsf_ProfitLossTextStudy(SCStudyInterfaceRef sc)
{
	...
	SCInputRef Input_DrawAboveMainPriceGraph = sc.Input[22];
	...
	Tool.Text = Text;
	Tool.DrawUnderneathMainGraph = Input_DrawAboveMainPriceGraph.GetYesNo() ? 0 : 1;
...
SCSFExport scsf_Sidewinder(SCStudyInterfaceRef sc)
{
	...
	float lsma_Angle = static_cast<float>(sc.SlopeToAngleInDegrees((Subgraph_Lsma[Pos]-((Subgraph_Lsma[Pos-1]+Subgraph_Lsma[Pos-2])/2.0f))*InverseTickSize));

	Subgraph_LsmaAngle[Pos] = lsma_Angle;

	// Calculate the EMA angle
	float ema_Angle	= static_cast<float>(sc.SlopeToAngleInDegrees((Subgraph_Ema[Pos]-((Subgraph_Ema[Pos-1]+Subgraph_Ema[Pos-2])/2.0f))*InverseTickSize));
...
SCSFExport scsf_MurreyMath(SCStudyInterfaceRef sc)
{
	...
	double HighestHighOfSquareWidthLength = static_cast<double>(sc.GetHighest(sc.High, PreviousStartOutputBarIndex + BarsBackToReference - 1, BarsBackToReference));

	double LowestLowOfSquareWidthLength = static_cast<double>(sc.GetLowest(sc.Low, PreviousStartOutputBarIndex + BarsBackToReference - 1, BarsBackToReference));
...
SCSFExport scsf_PremierStochasticOscillator(SCStudyInterfaceRef sc)
{
	...
	case 1:
	{
		sc.DataStartIndex = Input_FastKLength.GetInt() + Input_FastDLength.GetInt() + 2 * Input_PSOMovAvgType.GetInt() - 1;
	...
	case 2:
	{
		sc.DataStartIndex = Input_FastKLength.GetInt() + Input_FastDLength.GetInt() + Input_SlowDLength.GetInt() + 2 * Input_PSOMovAvgType.GetInt() - 1;
	...
	sc.MovingAverage(Array_Stochastic, Array_EMAStochastic, Input_PSOMovAvgType.GetMovAvgType(), Input_PSOLength.GetInt());
	sc.MovingAverage(Array_EMAStochastic, Array_EMAEMAStochastic, Input_PSOMovAvgType.GetMovAvgType(), Input_PSOLength.GetInt());
...
SCSFExport scsf_ConnorsRSI(SCStudyInterfaceRef sc)
{
	...
	//Compute RSI
	sc.RSI(sc.BaseDataIn[Input_InputData.GetInputDataIndex()], Subgraph_RSI, Input_RSIAvgType.GetMovAvgType(), Input_RSILength.GetInt());
	...
	else
	{
		...
		while (sc.BaseDataIn[Input_InputData.GetInputDataIndex()][sc.Index - i - 1] < sc.BaseDataIn[Input_InputData.GetInputDataIndex()][sc.Index - i])
		{
			UpDownStreak += 1.0f;
		...
		while (sc.BaseDataIn[Input_InputData.GetInputDataIndex()][sc.Index - j - 1] > sc.BaseDataIn[Input_InputData.GetInputDataIndex()][sc.Index - j])
		{
			UpDownStreak -= 1.0f;
	...
	Array_PriceChange[sc.Index] = sc.BaseDataIn[Input_InputData.GetInputDataIndex()][sc.Index] - sc.BaseDataIn[Input_InputData.GetInputDataIndex()][sc.Index - 1];
	...
	//Compute Connors RSI
	Subgraph_ConnorsRSI[sc.Index] = (Subgraph_RSI[sc.Index] + Subgraph_RSIUpDownLength[sc.Index] + Subgraph_RateOfChange[sc.Index]) / 3.0f;
...
/*==========================================================================*/
SCSFExport scsf_TradeMenuStatus(SCStudyInterfaceRef sc)
{
	SCSubgraphRef Subgraph_TradingIsLocked = sc.Subgraph[0];
	SCSubgraphRef Subgraph_GlobalTradeSimulationIsOn = sc.Subgraph[1];
	SCSubgraphRef Subgraph_ChartTradeModeOn = sc.Subgraph[2];

	if (sc.SetDefaults)
	{
		sc.GraphName = "Trade Menu Status";

		sc.GraphRegion = 0;
		sc.AutoLoop = 1;
		sc.ScaleRangeType = SCALE_SAMEASREGION;

		Subgraph_TradingIsLocked.Name = "Trading is Locked";
		Subgraph_TradingIsLocked.DrawStyle = DRAWSTYLE_IGNORE;

		Subgraph_GlobalTradeSimulationIsOn.Name = "Global Trade Simulation Is On";
		Subgraph_GlobalTradeSimulationIsOn.DrawStyle = DRAWSTYLE_IGNORE;

		Subgraph_ChartTradeModeOn.Name = "Chart Trade Mode On";
		Subgraph_ChartTradeModeOn.DrawStyle = DRAWSTYLE_IGNORE;
	...
	{
		Subgraph_TradingIsLocked[sc.Index] = 1.0f;
	}
	else
	{
		Subgraph_TradingIsLocked[sc.Index] = 0.0f;
	}

	if (sc.GlobalTradeSimulationIsOn)
	{
		Subgraph_GlobalTradeSimulationIsOn[sc.Index] = 1.0f;
	}
	else
	{
		Subgraph_GlobalTradeSimulationIsOn[sc.Index] = 0.0f;
	}

	if (sc.IsChartTradeModeOn)
	{
		Subgraph_ChartTradeModeOn[sc.Index] = 1.0f;
	}
	else
	{
		Subgraph_ChartTradeModeOn[sc.Index] = 0.0f;
	}
}