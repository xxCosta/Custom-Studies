...
SCSFExport CustomChartBarBuildingFunction(SCCustomChartBarInterfaceRef ChartBarInterface)
{
	...
	//This is set to 0 by Sierra Chart upon each entry into this function, but just setting it to 0 here for clarity.
	ChartBarInterface.StartNewBarFlag = 0;


#ifdef _DEBUG
	// This code checks for skipped file record indexes and skipped chart bar indexes
	int64_t& r_FileRecordIndex = ChartBarInterface.GetPersistentInt64(1);
	int64_t& r_ChartBarIndex = ChartBarInterface.GetPersistentInt64(2);

	if (ChartBarInterface.IsBeginBarBuilding)
	{
		// Reset.
		r_FileRecordIndex = 0;
		r_ChartBarIndex = 0;
	...
	if (ChartBarInterface.NewFileRecordIndex != UINT64_MAX)
	{
		int64_t FileIndexDifference = ChartBarInterface.NewFileRecordIndex - r_FileRecordIndex;

		if (r_FileRecordIndex != 0 && FileIndexDifference > 1)
		{
			SCString MessageText("FileIndexDifference=");
			MessageText.AppendFormat("%d", static_cast<int>(FileIndexDifference));
			ChartBarInterface.AddMessageToLog(MessageText, 0);
		}

		r_FileRecordIndex = ChartBarInterface.NewFileRecordIndex;
	}

	{
		int64_t BarIndexDifference = ChartBarInterface.CurrentBarIndex - r_ChartBarIndex;

		if (BarIndexDifference > 1)
		{
			SCString MessageText("BarIndexDifference=");
			MessageText.AppendFormat("%d", static_cast<int>(BarIndexDifference));
			ChartBarInterface.AddMessageToLog(MessageText, 0);
		}

		r_ChartBarIndex = ChartBarInterface.CurrentBarIndex;
	}
	...
	{
		//This flag is set to 1 when this function is called after the initial processing of the intraday file record but before it is added to the most recent chart bar.  
