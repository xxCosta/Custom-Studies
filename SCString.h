...
class SCString
{
	...
	public:
		...
		void Copy(const char* String, int StringLength);
		...
		void ParseLineItemsAsFloats(std::vector<float>& r_FloatValues) const;
...
inline SCString::~SCString()
{
	if (m_OwnString && m_String != nullptr)
	{
...
{
	if ((String == nullptr || String[0] == '\0') && IsEmpty())
...
/*==========================================================================*/
inline void SCString::Copy(const char* String, int StringLength)
{
	Initialize();
...
inline int SCString::Compare(const char* StringToCompare, int NumChars) const
{
	...
	if (StringToCompare != nullptr)
	{
		if (NumChars == 0)
			return strcmp(InternalString, StringToCompare);
		else
			return strncmp(InternalString, StringToCompare, NumChars);
	}
	else 
		return 0;
...
inline int SCString::IndexOf(char Delimiter, int StartIndex) const
{
	...
	for (int CharacterIndex = StartIndex; CharacterIndex < GetLength(); CharacterIndex++)
	{
		if (m_String[CharacterIndex] == Delimiter)
...
/*============================================================================
	SubstringLength can be -1 and it will be automatically calculated.
----------------------------------------------------------------------------*/
...
/*==========================================================================*/
inline void SCString::ParseLineItemsAsFloats(std::vector<float>& r_FloatValues) const
{
	constexpr char Delimiter = ',';

	// Do not add any values if the string is empty.
	if (IsEmpty())
		return;
	...
	while (true)
	{
		const int FoundOffset = IndexOf(Delimiter, NextSearchOffset);

		if (FoundOffset < 0)
			break;

		const int Length = FoundOffset - NextSearchOffset;

		const SCString ValueString = GetSubString(Length, NextSearchOffset);

		r_FloatValues.push_back(static_cast<float>(atof(ValueString)));

		NextSearchOffset = FoundOffset + 1;
	}

	// Get the final value after the last delimiter.
	const SCString FinalValueString = GetSubString(-1, NextSearchOffset);

	r_FloatValues.push_back(static_cast<float>(atof(FinalValueString)));
}
